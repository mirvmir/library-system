...
import io.github.mirvmir.useCases.services.interfaces.ImportOrderCsvService;
...
public class DefaultImportOrderCsvService implements ImportOrderCsvService {
    ...
    public ImportOrderOutput execute(ImportOrderCsvInput input) {
        ...
        Map<Long, LocalDateTime> orderExpiresDate = new HashMap<>();
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(pathStr))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    LocalDateTime expiresDate = dateOrNull.equals("null") ?
                            null : LocalDateTime.parse(dateOrNull);
                    ...
                    LocalDateTime existingExpiresDate = orderExpiresDate.get(orderId);
                    ...
                    if (existingExpiresDate != null
                            && !existingExpiresDate.equals(expiresDate)) {
                        errors.add("Заказ " + orderId
                                + " имеет разные expiresDate: "
                                + existingExpiresDate + " и " + expiresDate);
                        continue;
                    }
                    orderCompletionDate.put(orderId, completionDate);
                    orderExpiresDate.put(orderId, expiresDate);
        ...
        for (Long orderId : orderBookIds.keySet()) {
            ...
            importedOrders.add(
                    new Order(
                            ...
                            orderExpiresDate.get(orderId)
                    )
            );
        }

        for (Order importedOrder : importedOrders) {
            if (!customerRepo.existUser(importedOrder.getCustomerId()))
